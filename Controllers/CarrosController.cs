...
namespace PROYJHOME2026.Controllers
{
    public class CarrosController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> ExportarExcel(string? buscar, string? estado, string? categoria)
        {
            var query = _context.Carros
                .Include(c => c.EmpleadosCarros).ThenInclude(ec => ec.Empleado)
                .AsQueryable();
            ...
            var columnas = new List<string> { "Placa", "Marca", "Modelo", "Año", "Categoría", "Ejes", "Color", "Estado", "Conductor" };
            var filas = carros.Select(c => new List<string> {
                c.Placa ?? "—",
                c.Marca ?? "—",
                c.Modelo ?? "—",
                c.FechaCarro?.Year.ToString() ?? "—",
                c.Categoria ?? "—",
                c.Ejes?.ToString() ?? "—",
                c.Color ?? "—",
                c.Estado ?? "—",
                c.EmpleadosCarros.FirstOrDefault()?.Empleado is var emp && emp != null
                    ? emp.nombre + " " + emp.paterno : "Sin conductor"
            }).ToList();
        
            return GenerarCsv(columnas, filas, "Vehiculos");
}
        ...
        [HttpGet]
        public async Task<IActionResult> ExportarPdf(string? buscar, string? estado, string? categoria)
        {
            var query = _context.Carros
                .Include(c => c.EmpleadosCarros).ThenInclude(ec => ec.Empleado)
                .AsQueryable();
            ...
            var columnas = new List<string> { "Placa", "Marca", "Modelo", "Año", "Categoría", "Ejes", "Estado", "Conductor" };
            var filas = carros.Select(c => new List<string> {
                c.Placa ?? "—",
                c.Marca ?? "—",
                c.Modelo ?? "—",
            ...
            return GenerarPdf("Vehículos", columnas, filas);
        }
